...
export const admissionSchema = z.object({
    first_name: z.string("First name is required.").nonempty("First name is required.").min(3, "First name must be at least 3 characters."),
    last_name: z.string("Last name is required.").nonempty("Last name is required.").min(3, "Last name is required."),
    ...
    school_name: z.string("School name is required.").nonempty("School name is required.").min(10, "School name must be at least 10 characters."),
    passed_year: z.string("Passed year is required.").nonempty("Passed year is required.").regex(/^\d{4}$/, { message: "Passed year must be a 4 digit year." }),
    gpa: z.number("GPA is required.").nonnegative("GPA cannot be negative."),
    applied_for: z.enum(["science", "management", "law"]).default("science"),
